...
import { deleteSeller, deleteSellers, updateStatus } from "@/action/seller";
import { toast, useToast } from "@/hooks/use-toast";
import { FormatDate } from "../timeformate/page";

const COLORS = ["#0088FE", "#00C49F", "#FFBB28", "#FF8042"];

export default function AllSellers({ users, userGrowthData }) {
  ...
  const deletemultiple = async () => {
    const result = await deleteSellers(selectedUsers);
    if (result.success) {
      toast({
        title: "Success",
        description: result.message,
            ...
            <TableRow key={user.id}>
              ...
              <TableCell>
                <div className="flex space-x-2">
                  <Link href={`/admin-dashboard/seller-details/${user.id}`}>
